...
@if(BB::isList($list))
    <table class="table table-responsive table-hover">
        ...
        <tr>
            @foreach($list->table['headers'] as $headItem)
                <th scope="col">{{ $headItem }}</th>
            @endforeach
        </tr>
        </thead>
        <tbody>
        @if(count($list->table['records']))
            @foreach($list->table['records'] as $record)
                <tr>
                    @foreach($record as $key => $recordItem)
                        @if(gettype($recordItem) == 'array')
                            ...
                            @if(isset($recordItem['td_custom_HTML_tag']))
                                {!! $recordItem['td_custom_HTML_tag'] !!}
                                @endif>
