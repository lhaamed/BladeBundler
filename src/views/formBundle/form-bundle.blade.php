@if(\lhaamed\BladeBundler\BB::isForm($formBundle))
    ...
    method="{{ $formBundle->method }}"
          {!! $formBundle->custom_html_tags !!} class="@if(\lhaamed\BladeBundler\BB::isSearchForm($formBundle)) search-bundler @else form-bundler @endif">
            ...
            <nav class="tab-navigator mb-4 mx-2">
                <div class="nav nav-pills gap-2 pb-3 px-3" id="tab-navigator" role="tablist">
                    @foreach($formBundle->tabs as $key => $tab)
                        @if($tab instanceof \lhaamed\BladeBundler\classes\formBundle\partials\Tab)
